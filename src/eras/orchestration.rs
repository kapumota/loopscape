...
pub fn setup_orchestration_era(mut commands: Commands, mut metrics: ResMut<Metrics>) {
    metrics.active_loops = 0;
    metrics.era_timer = 0.0;

    // Crea 1 supervisor con 5 trabajadores.
    let supervisor_entity = commands
        .spawn((
            LoopAgent,
        ...
        let worker_entity = commands
            .spawn((
                ...
                ConsensusVoter {
                    ...
                    election_timeout: Timer::from_seconds(3.0 + (i as f32 * 0.5), TimerMode::Once),
                },
                ByzantineSuspect {
                    trust_score: 1.0,
...
pub fn heartbeat_system(
    ...
    for (_supervisor_entity, mut supervisor, supervisor_transform) in supervisors.iter_mut() {
        ...
        if supervisor.heartbeat_interval.just_finished() {
            ...
            let worker_entities = supervisor.workers.clone();
...
pub fn byzantine_detection(
    ...
    if keyboard.just_pressed(KeyCode::KeyB) {
        ...
        if let Some(mut suspect) = suspects.iter_mut().next() {
            suspect.trust_score -= 0.6;
            suspect.inconsistency_count += 1;
        }
    }
}

pub fn byzantine_visuals(mut suspects: Query<(&ByzantineSuspect, &mut LoopVisual)>) {
